...
export default {
  ...
  layout: 'admin',
  async created() {
    this.$store.dispatch('product/paginationData', {
  ...
  methods: {
    ...
    // activeProduct(data) {
    //   this.$store.dispatch('product/activeProduct', {
    ...
    search() {
      ...
      this.$store.dispatch('product/search', this.$refs.search.value)
    },
    deleteData({ id }) {
      this.$store.dispatch('product/deleteData', id)
    },
    search() {
      if (!this.$refs.search.value) {
        return
