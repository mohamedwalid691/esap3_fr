  ...
  <div class="admin-content col-10 no-gutters">
    ...
    <div class="container mt-5">
      <h1 class="text-capitalize h3 text-body">
        {{ $t('edit') }} {{ $t('productColor') }}
      </h1>
      <div class="d-flex justify-content-center">
        <button
          @click="editProductColor"
          type="button"
...
export default {
  ...
  async created() {
    console.log("this is color id");
    console.log(this.$route.params.id);
    await this.$store.dispatch('product/getSingleData', {
      type: 'color',
  ...
  methods: {
    async editProductColor() {
      await this.$store.dispatch('product/editData', {
        type: 'color',
        productId: this.$route.params.id,
        typeId: this.$route.params.color_id,
      })
    },
